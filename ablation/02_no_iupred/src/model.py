"""
Ablation: IUPred3 input zeroed out.
Model architecture identical to the original — only the IUPred3 features
are replaced with zero tensors during training/inference.
"""
...
class ResidueLLPSClassifier(nn.Module):
    """Residue-level LLPS classifier (identical architecture to original).
    ...
    iupred_scores : torch.Tensor, shape ``(B, L_max)``
        IUPred3 per-residue disorder scores (zeroed out in this ablation).
