"""
Train the residue-level LLPS classifier (one-hot + IUPred3 ablation).
Only change: ESM2 embeddings replaced by one-hot encoding.
"""
...
def train_step(
    model, seq_embeds, iupred_scores, lengths, labels,
    criterion, optimizer, scaler, grad_clip,
):
    model.train()
    optimizer.zero_grad()
    with autocast("cuda"):
        logits = model(seq_embeds, iupred_scores, lengths)
...
def validate(model, loader, criterion, device, extractor, return_raw=False):
    ...
    with torch.no_grad():
        for seqs, iupred_scores, lengths, labels in tqdm(loader, desc="Evaluating"):
            seq_embeds, _ = extractor.extract_residue_batch_tensors(
...
def main():
    ...
    print("=" * 60)
    print("LLPS Model Training (Ablation: One-hot + IUPred3)")
    ...
    onehot_extractor = OneHotExtractor()
    ...
    model = ResidueLLPSClassifier(d_alphabet=cfg.d_alphabet, hidden=cfg.hidden).to(device)
    ...
    for step in range(1, cfg.total_steps + 1):
        ...
        seq_embeds, _ = onehot_extractor.extract_residue_batch_tensors(
            seqs, pad_to_length=iupred_scores.size(1)
        )
        seq_embeds = seq_embeds.to(device)
        iupred_scores = iupred_scores.to(device)
        ...
        if step % cfg.eval_interval == 0:
            ...
            val_metrics = validate(model, val_loader, criterion, device, onehot_extractor)
    ...
    test_metrics, test_logits, test_labels_t = validate(
        model, test_loader, criterion, device, onehot_extractor, return_raw=True
    )
    print(
