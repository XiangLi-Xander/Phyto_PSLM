"""
Train the residue-level LLPS classifier (ESM2 zeroed out ablation).
Model architecture unchanged — ESM2 input is set to zero.
"""
...
def validate(model, loader, criterion, device, return_raw=False):
    """ESM2 embeddings are zeroed out in this ablation."""
    model.eval()
    total_loss = 0.0
    all_logits = []
    all_labels = []
    ...
    with torch.no_grad():
        for seqs, iupred_scores, lengths, labels in tqdm(loader, desc="Evaluating"):
            B, L_max = iupred_scores.size(0), iupred_scores.size(1)
            esm_embeds = torch.zeros(B, L_max, 1280, device=device)
...
def main():
    ...
    print("=" * 60)
    print("LLPS Model Training (Ablation: ESM2 zeroed out)")
    ...
    for step in range(1, cfg.total_steps + 1):
        ...
        B, L_max = iupred_scores.size(0), iupred_scores.size(1)
        esm_embeds = torch.zeros(B, L_max, cfg.d_model, device=device)
        iupred_scores = iupred_scores.to(device)
        lengths = lengths.to(device)
        labels = labels.to(device)
