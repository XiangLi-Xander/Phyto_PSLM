...
This module provides core building blocks shared across the pipeline:
sequence processing, ESM2 feature extraction, the PyTorch Dataset class,
collation logic, and evaluation metrics.
"""
...
def clean_sequence_df(
    ...
    """Clean and standardise a DataFrame of protein sequences.

    Steps
    -----
    1. Drop rows with missing sequences.
    2. Strip whitespace and convert to uppercase.
    3. Remove sequences containing non-standard amino acids.
    4. Filter by minimum length.
    5. Remove duplicate sequences.
    6. Optionally attach a label column.
    """
    if valid_aa is None:
...
class ESM2Extractor:
    ...
    def extract_cls_batch_tensors(self, seqs, max_len: int = MAX_LEN) -> torch.Tensor:
        """Extract ``<cls>`` features; returns ``(B, 1280)`` on GPU."""
    ...
    def extract_cls_batch(
        ...
        """Extract ``<cls>`` features; returns ``(N, 1280)`` numpy array."""
        all_feats = []
    ...
    def extract_residue_batch_tensors(
        ...
        """Extract residue-level features for a batch.

        Only the hidden states of actual residues are extracted
        (``<cls>``, ``<eos>`` and ``<pad>`` tokens are excluded).
        The output is padded to the maximum length within the batch
        (or to ``pad_to_length`` if provided).

        Returns
        -------
        residue_embeds : torch.Tensor
            Shape ``(B, L_max, 1280)`` on GPU; padding positions are zero.
        lengths : torch.Tensor
            Shape ``(B,)``, actual residue counts.
        """
        inputs = self.tokenizer(
            list(seqs),
            truncation=True,
    ...
    def extract_residue_batch(self, sequences, max_len: int = MAX_LEN, batch_size: int = 16):
        """Extract residue-level features for many sequences.

        Returns
        -------
        residue_embeds : list[np.ndarray]
            List of ``(L_i, 1280)`` arrays.
        lengths : list[int]
...
def collate_fn(batch):
    ...
    IUPred3 scores are padded to the maximum length within the batch.
    """
...
def compute_metrics(logits: torch.Tensor, labels: torch.Tensor) -> dict:
    ...
    Returns a dictionary with keys ``acc``, ``precision``, ``recall``,
    ``f1``, ``auc``, ``aupr``.
    """
