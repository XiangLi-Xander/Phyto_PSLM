"""
Train the residue-level LLPS classifier.

Step-based training loop (no fixed epochs) that:
- Loads pre-processed CSV data (train/val/test).
- Computes IUPred3 per-residue scores on-the-fly.
- Extracts ESM2 per-residue embeddings per batch on GPU.
- Uses ``BCEWithLogitsLoss`` with no class weighting.
- Validates at regular intervals using F1-score.
- Implements early stopping (patience of five evaluations).
- Evaluates on the held-out test set after training.
"""
...
class Config:
    def __init__(self):
        ...
        self.model_dir = os.path.join(self.base_dir, "outputs", "models")
        self.log_dir = os.path.join(self.base_dir, "outputs", "logs")
        self.esm2_dir = os.path.join(self.base_dir, "esm2")
...
def log_gpu_stats():
    """Print current GPU memory usage and utilisation."""
    try:
        import subprocess
...
def log_model_gpu_usage(model: nn.Module, label: str = "model"):
    """Estimate GPU memory occupied by model parameters."""
...
def validate(model, loader, criterion, device, extractor, return_raw=False):
    """Evaluate the model over a full validation or test set."""
    model.eval()
    total_loss = 0.0
    all_logits = []
    all_labels = []
...
def main():
    ...
    print("=" * 60)
    print("LLPS Model Training (Single-GPU, Step-based)")
